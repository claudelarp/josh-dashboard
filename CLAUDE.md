...
- **Tasks:** `- [ ] task (added YYYY-MM-DD) !due:YYYY-MM-DD` (due tag optional). Any session that surfaces a task writes it to the right domain's `tasks.md`; done means checked off, not deleted. These files are also the **seed source** for the dashboard (below).
- **Dashboard:** `dashboard.html` at the vault root is a **self-contained interactive app** (opens by double-click; dark "Jarvis" aesthetic; goal ticker, day ring, today/tomorrow to-do, streaks, per-domain tabs with backlogs, a Stats tab with trend/donut/heatmap/rings, custom habit trackers, and a full Settings modal for fonts/colors/domains/trackers/tabs/widgets). **Its state lives in the browser (localStorage), not in files** — Claude cannot read or edit what's in it. "Refresh the dashboard" now means *reload the page in the browser*.
- **Seed bridge (files → dashboard, one-way):** `python3 scripts/build-dashboard.py` reads open tasks from `projects/*/tasks.md` and injects them into the `<!--SEED-->…<!--/SEED-->` block in `dashboard.html`. On next page load the app merges unseen items (deduped by hash; due-today→Today, else→domain backlog) — tasks deleted in the browser stay deleted; editing a task's text re-imports it as new. **The script only rewrites between those markers and refuses to run if they're missing** — it never regenerates the app. "Push my tasks to the dashboard" = run that script. There is no dashboard→files path (by design; the dashboard is Josh's private live surface).
- Projects map to wiki domains for durable knowledge: business→`business`, fitness-health→`fitness`/`health`, marketing→`media`, school→`schoolwork`, self-development→`habits`. Dashboard domain ids match the `projects/` folder names.
